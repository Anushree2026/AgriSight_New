...
def generate_frames():
    global camera_running, last_detected, last_saved_time

    # 🔥 OPEN CAMERA ONCE (VERY IMPORTANT)
    ...
    # 🔥 SET PROPERTIES (IMPORTANT FOR PI)
    ...
    while True:
        ...
        # 🔥 HARD FIX FOR YOUR ISSUE
        if not ret or frame is None:
            print("⚠️ Frame failed, restarting camera...")
        ...
        for r in results:
            for box in r.boxes:
                ...
                if (class_name != last_detected) or (current_time - last_saved_time > SAVE_INTERVAL):
                    try:
                        current_time = time.time()
                        ...
                        if (class_name != last_detected) or (current_time - last_saved_time > SAVE_INTERVAL):
                            try:
                                with app.app_context():   # ✅ FIX HERE
                                    ...
                                    new_detection = Detection(detected_value=class_name)
